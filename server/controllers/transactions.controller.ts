...
/** Normalises whatever the client sent into one of the three supported cycles. */
export const normaliseBillingCycle = (value: unknown): BillingCycle => {
...
export const createTransaction = async (req: Request, res: Response) => {
  try {
    ...
    const normalisedCycle = normaliseBillingCycle(billingCycle);
    const amount = planPriceForCycle(plan, normalisedCycle, requestedCurrency);
...
export const initiatePayment = async (req: Request, res: Response) => {
  try {
    ...
    const plan = planData[0];
    const cycle = normaliseBillingCycle(billingCycle);
    const requestedCurrency = currency ? String(currency).toUpperCase() : null;
    const amount = planPriceForCycle(plan, cycle, requestedCurrency);
