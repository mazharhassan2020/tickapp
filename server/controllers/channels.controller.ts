...
export const getChannels = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (user.role === 'superadmin') {
    ...
    // Admin sees their own channels; a team member works on their admin's
    const ownerId = user.role === 'team' ? (user.createdBy || user.id) : user.id;
    channels = await storage.getChannelsByUser(ownerId);
  }
...
export const createChannel = asyncHandler(async (req: Request, res: Response) => {
  ...
  let createdBy = sessionUser?.id || 'unknown';

  // A superadmin may create the channel on behalf of another account
  // (Users → user details → Channels → Add Channel).
  const ownerUserId = req.body?.ownerUserId;
  if (ownerUserId && ownerUserId !== createdBy) {
  ...
  // Per-account channel allowance (superadmin sets it from Users → channel count).
  ...
  if (sessionUser?.role !== 'superadmin') {
    ...
    if (existing.length >= limit) {
      return res.status(403).json({
        error: `Channel limit reached. Your account can connect ${limit} channel(s). Contact support to add more.`,
        limit,
...
export const embeddedSignup = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (user.role !== "superadmin") {
      ...
      if (existingChannels.length >= limit) {
        await updateLog({ status: "failed", step: "channel_limit" });
        return res.status(403).json({
          message: `Channel limit reached. Your account can connect ${limit} channel(s). Contact support to add more.`,
          limit,
          current: existingChannels.length,
        });
      }
    }
...
export const updateChannel = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Superadmins may edit any user's channel; everyone else only their own
  // (team members act on the channels of the account that created them).
  const ownerId = user?.role === 'team' ? user?.createdBy : userId;
  if (user?.role !== 'superadmin' && existing.createdBy !== ownerId && existing.createdBy !== userId) {
    return res.status(403).json({ error: 'Access denied' });
  }

  // If setting this channel as active, deactivate all others of the owner
  if (req.body.isActive === true) {
    const channels = await storage.getChannelsByUserId(existing.createdBy || userId);
    for (const channel of channels) {
  ...
  // Never let the payload move a channel to another owner
  const { id: _ignoredId, createdBy: _ignoredOwner, ...updates } = req.body || {};
  ...
  const credentialsChanged = "accessToken" in updates || "whatsappBusinessAccountId" in updates;
