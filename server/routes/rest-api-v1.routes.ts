...
export function registerRestApiV1Routes(app: Express) {
  app.post("/api/v1/messages/template", requireApiKey, requirePermission("messages.send"), async (req: Request, res: Response) => {
    try {
      ...
      try {
        // Resolve the template category for correct per-category billing.
        const [tmpl] = await db
          .select({ category: schema.templates.category })
        ...
        const result = await billedSendTemplate({
          userId: (channel as any).createdBy || userId,
          channel,
          to,
          templateName,
        ...
        if (sendError instanceof InsufficientBalanceError) {
