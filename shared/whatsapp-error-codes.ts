...
const ERROR_MAP: Record<number, WhatsAppErrorInfo> = {
  ...
  200: {
    ...
    description:
      "Meta will not let this app send on behalf of this WhatsApp Business Account. This is almost always a client-owned WABA (onboarded via Embedded Signup) while the app only has Standard Access.",
    suggestion:
      "In the Meta App Dashboard: set the app to Live mode and request Advanced Access for whatsapp_business_messaging (needs Business Verification + Tech Provider onboarding). Until then, only numbers on a WABA owned by your own business can send — or move this number into your own WABA. Re-running the signup flow alone will not fix it.",
    category: "authentication",
  },
  368: {
    code: 368,
